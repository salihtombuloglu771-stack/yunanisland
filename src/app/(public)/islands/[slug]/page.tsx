...
import { getIslandBySlug, getBeachesForIsland, getRestaurantsForIsland } from '@/lib/mockData'
...
export default function IslandPage({ params }: PageProps) {
  // Next.js 16/React 19 asynchronous params resolution in client components
  const { slug } = use(params)
  
  const island = getIslandBySlug(slug)
  const allBeaches = island ? getBeachesForIsland(island.id) : []
  const allRestaurants = island ? getRestaurantsForIsland(island.id) : []
  ...
  useEffect(() => {
    if (!island || !island.latitude || !island.longitude) return
      ...
      <header className="sticky top-0 z-50 w-full border-b border-slate-100 dark:border-neutral-900 bg-white/80 dark:bg-neutral-950/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl h-16 items-center justify-between px-6">
          ...
          <div className="flex items-center gap-4">
            <Link href="/" className="text-sm font-semibold text-neutral-600 dark:text-neutral-400 hover:text-sky-600 dark:hover:text-sky-400 transition-colors">
              ← Tüm Adalar
            </Link>
          </div>
        </div>
      ...
      <main className="mx-auto max-w-7xl px-6 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {activeTab === 'restaurants' && (
                <div>
                  ...
                  <div className="bg-white dark:bg-neutral-900 p-5 rounded-2xl border border-slate-100 dark:border-neutral-900 shadow-sm mb-6">
                    ...
                    {/* Checkbox Restoran Tesis Filtreleri */}
          ...
          <div className="space-y-6">
            ...
            {/* Hava Durumu Simülasyonu / Canlı API */}
            <div className="bg-white dark:bg-neutral-900 p-6 rounded-2xl border border-slate-100 dark:border-neutral-900 shadow-sm">
              <h3 className="text-sm font-bold text-neutral-400 dark:text-neutral-500 uppercase tracking-wider mb-4">
